...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'<div class="layui-layim-info" style="height: 60px;margin-bottom: 10px;">'
    ,'<div style="height: 60px;width: 120px;margin-bottom: 10px;display: flex;flex-direction: row;flex-wrap: nowrap;align-items: center;justify-content: space-around;">'
      ,'<div style="width:50px;height:50px;border-radius:50%;display: flex;justify-content: center;overflow: hidden;align-items: center;"><img style="width:100%;" src="{{d.mine.avatar}}" ></div>'
      ,'<div class="layui-layim-user" style="max-width: 65px;">{{ d.mine.username }}</div>'
    ,'</div>'
    ...
    ,'<ul style="display:none;" class="layui-unselect layui-layim-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}'
      ,' layim-tab-two'
    ,'{{# } }}">'
      ,'<li class="layui-icon'
      ,'{{# if(!d.base.isfriend){ }}'
      ...
      ,'<li  title="历史会话" layim-event="tab" lay-type="history"></li>'
    ,'</ul>'
    ,'<ul style="height:428px;" class="layui-unselect layim-tab-content {{# if(d.base.isfriend){ }}layui-show{{# } }} layim-list-friend">'
    ...
    ,'<ul class="layui-unselect layim-tab-content {{# if(!d.base.isfriend && d.base.isgroup){ }}layui-show{{# } }}">'
      ...
      ,'</li>'
    ,'</ul>'
    ,'<ul style="height:382px;" class="layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }}">'
      ,'<li>'
    ...
    ,'<ul class="layui-unselect layui-layim-tool">'
      ,'<li class="layui-icon layim-tool-search" layim-event="search" title="搜索" style="display:none;">&#xe615;</li>'
      ,'{{# if(d.base.msgbox){ }}'
      ,'<li class="layui-icon layim-tool-msgbox" layim-event="msgbox" title="消息盒子">&#xe645;<span class="layui-anim"></span></li>'
      ,'{{# } }}'
      ,'{{# if(d.base.find){ }}'
      ,'<li class="layui-icon layim-tool-find" layim-event="find" title="查找" style="display:none;">&#xe608;</li>'
      ,'{{# } }}'
      ,'<li class="layui-icon layim-tool-skin" layim-event="skin" title="更换背景">&#xe61b;</li>'
      ,'{{# if(!d.base.copyright){ }}'
      ,'<li class="layui-icon layim-tool-about" layim-event="about" title="关于" style="display:none;">&#xe60b;</li>'
      ,'{{# } }}'
    ,'</ul>'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ,'<div class="layui-unselect layim-chat-title">'
      ,'<div class="layim-chat-other">'
        ,'<img id="current-chat-man" data-name="{{d.data.name}}" data-id="{{d.data.id}}" class="layim-{{ d.data.type }}{{ d.data.id }}" src="{{ d.data.avatar }}"><span class="layim-chat-username" layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||"佚名" }}  {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>'
        ,'<p class="layim-chat-status"><span style="display:block;">{{d.data.sign}}</span></p>'
      ,'</div>'
    ,'</div>'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'{{# }; }}'
      ,'</div>'
      ,'<div class="layim-chat-textarea"><textarea style="border: solid 1px #FFF !important;box-shadow: 0px 0px 8px #FFF;-moz-box-shadow: 0px 0px 8px #FFF;-webkit-box-shadow: 0px 0px 8px #FFF;"></textarea></div>'
  ...
  var elemChatList = '<li data-id="{{ d.data.id }}" class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<span class="friend-count" id="friend{{d.data.id}}" style="width: 20px;height: 20px;line-height: 20px;margin: 10px 0px 0 0;background-color: #1e9fff;padding: 1px;color: white;border-radius: 50%;display:none;text-align:center;"></span><i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  var setChatMin = function(newMsg){
    ...
    setChatMin.index = layer.open({
      ...
      ,offset: 'b'
      ,movemove: '#layui-layim-min'
      ,resize: false
      ,area: ['182px', '50px']
      ,content: '<img id="layui-layim-min" src="'+ thatChat.avatar +'"><span class="span-showChat" data-id="'+newMsg.id+'" data-name="'+newMsg.id+'">'+ thatChat.name +'</span>'
      ,success: function(layero, index){
  ...
  var changeChat = function(elem, del){
    ...
    // var id = $(".layim-chat-box .layui-show #current-chat-man").attr("data-id")
    //     ? $(".layim-chat-box .layui-show #current-chat-man").attr("data-id")
    //     : null;
    //   localStorage.setItem("current-chat-man", id);
  };
    ...
    if((!layimChat && data.content) || index === -1){
      ...
      if(!data.system){
        ...
        return setChatMin({
          name: '收到新消息'
          ,avatar: group.avatar || data.avatar
          ,anim: 6, id: data.id, username: data.username
        });
      }
    }
