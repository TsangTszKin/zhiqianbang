...
Guacamole.windowHeight = function () {
    let windowHeight = 0;
...
Guacamole.windowWidth = function () {
    let windowWidth = 0;
...
Guacamole.Client = function (h) {
    ...
    var p = [];
    var A = [];
    var D = new Guacamole.IntegerPool();
    var I = [];
    ...
    this.sendSize = function (L, K) {
        if (!w()) {
            return
        }
        h.sendMessage("size", L, K)
    };
    this.sendKeyEvent = function (K, L) {
        if (!w()) {
            return
        }
        h.sendMessage("key", L, K)
    };
    this.sendMouseState = function (L) {
        if (!w()) {
            return
        }
        s.moveCursor(Math.floor(L.x), Math.floor(L.y));
        var K = 0;
        if (L.left) {
            K |= 1
        }
        ...
        h.sendMessage("mouse", Math.floor(L.x), Math.floor(L.y), K)
    };
    this.setClipboard = function (M) {
        if (!w()) {
            return
        }
        var N = o.createClipboardStream("text/plain");
        var L = new Guacamole.StringWriter(N);
        for (var K = 0; K < M.length; K += 4096) {
            L.sendText(M.substring(K, K + 4096))
        }
        L.sendEnd()
    };
    this.createOutputStream = function l() {
        var K = D.next();
        var L = I[K] = new Guacamole.OutputStream(o, K);
        return L
    };
    this.createAudioStream = function (K) {
        var L = o.createOutputStream();
        h.sendMessage("audio", L.index, K);
        return L
    };
    this.createFileStream = function (K, L) {
        var M = o.createOutputStream();
        h.sendMessage("file", M.index, K, L);
        return M
    };
    this.createPipeStream = function (K, L) {
        var M = o.createOutputStream();
        h.sendMessage("pipe", M.index, K, L);
        return M
    };
    this.createClipboardStream = function (K) {
        var L = o.createOutputStream();
        h.sendMessage("clipboard", L.index, K);
        return L
    };
    this.createObjectOutputStream = function n(M, K, L) {
        var N = o.createOutputStream();
        h.sendMessage("put", M, N.index, K, L);
        return N
    };
    this.requestObjectInputStream = function d(L, K) {
        if (!w()) {
            return
        }
        h.sendMessage("get", L, K)
    };
    this.sendAck = function (K, M, L) {
        if (!w()) {
            return
        }
        h.sendMessage("ack", K, M, L)
    };
    this.sendBlob = function (K, L) {
        if (!w()) {
            return
        }
        h.sendMessage("blob", K, L)
    };
    this.endStream = function (K) {
        if (!w()) {
            return
        }
        h.sendMessage("end", K);
    ...
    function g(K) {
        var L = k[K];
        if (L == null) {
            L = k[K] = new Guacamole.Parser();
            L.oninstruction = h.oninstruction
        }
        return L
    }
    var a = {
        "miter-limit": function (K, L) {
            s.setMiterLimit(K, parseFloat(L))
        }
    };
    var f = {
        ack: function (L) {
            ...
            if (N) {
                if (N.onack) {
                    N.onack(new Guacamole.Status(K, M))
                }
                if (K >= 256 && I[O] === N) {
                    D.free(O);
                    delete I[O]
        ...
        arc: function (Q) {
            ...
            var R = parseInt(Q[2]);
            var K = parseInt(Q[3]);
            var O = parseFloat(Q[4]);
            var M = parseFloat(Q[5]);
            var N = parseInt(Q[6]);
            s.arc(P, L, R, K, O, M, N != 0)
        },
        audio: function (M) {
            var O = parseInt(M[0]);
            var L = M[1];
            var N = p[O] = new Guacamole.InputStream(o, O);
            var K = null;
            if (o.onaudio) {
                K = o.onaudio(N, L)
            }
            if (!K) {
                K = Guacamole.AudioPlayer.getInstance(N, L)
            }
            if (K) {
                u[O] = K;
                o.sendAck(O, "OK", 0)
            } else {
                o.sendAck(O, "BAD TYPE", 783)
            }
        },
        blob: function (K) {
            var N = parseInt(K[0]);
        ...
        body: function i(N) {
            ...
            var L = N[3];
            if (M && M.onbody) {
                var P = p[Q] = new Guacamole.InputStream(o, Q);
                M.onbody(P, K, L)
            } else {
                o.sendAck(Q, "Receipt of body unsupported", 256)
            }
        },
        cfill: function (P) {
            var Q = parseInt(P[0]);
            var M = b(parseInt(P[1]));
            var O = parseInt(P[2]);
            var N = parseInt(P[3]);
            var K = parseInt(P[4]);
            var L = parseInt(P[5]);
            s.setChannelMask(M, Q);
            s.fillColor(M, O, N, K, L)
        },
        clip: function (L) {
            var K = b(parseInt(L[0]));
            s.clip(K)
        },
        clipboard: function (L) {
            var N = parseInt(L[0]);
            var K = L[1];
            if (o.onclipboard) {
                var M = p[N] = new Guacamole.InputStream(o, N);
                o.onclipboard(M, K)
            } else {
                o.sendAck(N, "Clipboard unsupported", 256)
            }
        },
        close: function (L) {
            var K = b(parseInt(L[0]));
        ...
        copy: function (S) {
            var K = b(parseInt(S[0]));
            var O = parseInt(S[1]);
            var N = parseInt(S[2]);
            var M = parseInt(S[3]);
            var T = parseInt(S[4]);
            var R = parseInt(S[5]);
            var L = b(parseInt(S[6]));
            var Q = parseInt(S[7]);
            var P = parseInt(S[8]);
            s.setChannelMask(L, R);
            s.copy(K, O, N, M, T, L, Q, P)
        },
        cstroke: function (S) {
            var P = parseInt(S[0]);
            var N = b(parseInt(S[1]));
            var T = j[parseInt(S[2])];
            var L = H[parseInt(S[3])];
            var R = parseInt(S[4]);
            var K = parseInt(S[5]);
            var M = parseInt(S[6]);
            var O = parseInt(S[7]);
            var Q = parseInt(S[8]);
            s.setChannelMask(N, P);
            s.strokeColor(N, T, L, R, K, M, O, Q)
        },
        cursor: function (R) {
            var Q = parseInt(R[0]);
            var P = parseInt(R[1]);
            var N = b(parseInt(R[2]));
            var L = parseInt(R[3]);
            var K = parseInt(R[4]);
            var O = parseInt(R[5]);
            var M = parseInt(R[6]);
            s.setCursor(Q, P, N, L, K, O, M)
        },
        curve: function (Q) {
            var P = b(parseInt(Q[0]));
            var M = parseInt(Q[1]);
            ...
            var K = parseInt(Q[5]);
            var R = parseInt(Q[6]);
            s.curveTo(P, M, L, O, N, K, R)
        },
        disconnect: function e(K) {
            o.disconnect()
        },
        dispose: function (M) {
            var K = parseInt(M[0]);
        ...
        distort: function (S) {
            ...
            var P = parseFloat(S[3]);
            var O = parseFloat(S[4]);
            var N = parseFloat(S[5]);
            var M = parseFloat(S[6]);
            if (K >= 0) {
                var L = b(K);
                s.distort(L, R, Q, P, O, N, M)
            }
        },
        error: function (L) {
            var M = L[0];
            var K = parseInt(L[1]);
            if (o.onerror) {
                o.onerror(new Guacamole.Status(K, M))
            }
            o.disconnect()
        },
        end: function (K) {
        ...
        file: function (M) {
            ...
            if (o.onfile) {
                ...
                o.sendAck(O, "File transfer unsupported", 256)
            }
        },
        filesystem: function y(M) {
            var N = parseInt(M[0]);
            var L = M[1];
            if (o.onfilesystem) {
                var K = A[N] = new Guacamole.Object(o, N);
                o.onfilesystem(K, L)
            }
        },
        identity: function (L) {
            var K = b(parseInt(L[0]));
            s.setTransform(K, 1, 0, 0, 1, 0, 0)
        },
        img: function (T) {
            var K = parseInt(T[0]);
            var O = parseInt(T[1]);
            var N = b(parseInt(T[2]));
            var Q = T[3];
            var R = parseInt(T[4]);
            var P = parseInt(T[5]);
            var S = p[K] = new Guacamole.InputStream(o, K);
            var M = new Guacamole.DataURIReader(S, Q);
            M.onend = function L() {
                s.setChannelMask(N, O);
                s.draw(N, R, P, M.getURI())
            }
        },
        jpeg: function (M) {
            var P = parseInt(M[0]);
            var L = b(parseInt(M[1]));
            var K = parseInt(M[2]);
            var O = parseInt(M[3]);
            var N = M[4];
            s.setChannelMask(L, P);
            s.draw(L, K, O, "data:image/jpeg;base64," + N)
        },
        lfill: function (M) {
            var N = parseInt(M[0]);
            var L = b(parseInt(M[1]));
            var K = b(parseInt(M[2]));
            s.setChannelMask(L, N);
            s.fillLayer(L, K)
        },
        line: function (M) {
            var L = b(parseInt(M[0]));
            var K = parseInt(M[1]);
            var N = parseInt(M[2]);
            s.lineTo(L, K, N)
        },
        ...
        pipe: function (M) {
            ...
            if (o.onpipe) {
                ...
                o.sendAck(O, "Named pipes unsupported", 256)
            }
        },
        png: function (M) {
            var P = parseInt(M[0]);
            ...
            var N = M[4];
            s.setChannelMask(L, P);
            s.draw(L, K, O, "data:image/png;base64," + N)
        },
        pop: function (L) {
            var K = b(parseInt(L[0]));
            s.pop(K)
        ...
        transfer: function (S) {
            ...
            var Q = parseInt(S[8]);
            if (M === 3) {
                s.put(K, P, O, N, T, L, R, Q)
            } else {
                if (M !== 5) {
                    s.transfer(K, P, O, N, T, L, R, Q, Guacamole.Client.DefaultTransferFunction[M])
                }
            }
        },
        transform: function (N) {
            var M = b(parseInt(N[0]));
            var L = parseFloat(N[1]);
            var K = parseFloat(N[2]);
            var R = parseFloat(N[3]);
            var Q = parseFloat(N[4]);
            var P = parseFloat(N[5]);
            var O = parseFloat(N[6]);
            s.transform(M, L, K, R, Q, P, O)
        },
        undefine: function F(L) {
            var M = parseInt(L[0]);
            var K = A[M];
            if (K && K.onundefine) {
                K.onundefine()
        ...
        video: function (N) {
            var P = parseInt(N[0]);
            var L = b(parseInt(N[1]));
            var K = N[2];
            var O = p[P] = new Guacamole.InputStream(o, P);
            var M = null;
            if (o.onvideo) {
                M = o.onvideo(O, L, K)
            }
            if (!M) {
                M = Guacamole.VideoPlayer.getInstance(O, L, K)
            }
            if (M) {
                C[P] = M;
                o.sendAck(P, "OK", 0)
            } else {
                o.sendAck(P, "BAD TYPE", 783)
            }
        }
    };
    h.oninstruction = function (M, L) {
        var K = f[M];
        if (K) {
            K(L)
        }
...
Guacamole.Display = function () {
    ...
    // e.getElement().style.minWidth = Guacamole.windowWidth() + "px";
...
Guacamole.Display.VisibleLayer = function (f, a) {
    ...
    d.style.left = "0px";
    d.style.top = "0px";
    // d.style.width = Guacamole.windowWidth() + "px";
    // d.style.height = Guacamole.windowHeight() + "px";
    var h = document.createElement("div");
    h.appendChild(d);
    h.style.width = f + "px";
    h.style.height = a + "px";
...
Guacamole.Mouse = function (f) {
    ...
    f.addEventListener("contextmenu", function (i) {
        d(i)
    }, false);
    f.addEventListener("mousemove", function (i) {
        d(i);
        if (g) {
            g--;
            return
    ...
    f.addEventListener("mousedown", function (i) {
        d(i);
        if (g) {
            return
        }
    ...
    f.addEventListener("mouseup", function (i) {
        d(i);
        if (g) {
            return
        }
    ...
    f.addEventListener("mouseout", function (j) {
        if (!j) {
            j = window.event
        }
    ...
    f.addEventListener("selectstart", function (i) {
        d(i)
    }, false);
...
Guacamole.Mouse.Touchpad = function (d) {
    ...
    var j = false;
    var i = null;
    d.addEventListener("touchend", function (m) {
        m.preventDefault();
    ...
    d.addEventListener("touchstart", function (l) {
        l.preventDefault();
    ...
    d.addEventListener("touchmove", function (m) {
        m.preventDefault();
        var q = m.touches[0];
        var p = q.clientX - h;
        var o = q.clientY - g;
...
Guacamole.Mouse.Touchscreen = function (e) {
    ...
    e.addEventListener("touchstart", function (o) {
    ...
    e.addEventListener("touchmove", function (o) {
        if (!k) {
            return
        }
        if (a(o)) {
...
Guacamole.OnScreenKeyboard = function (q) {
    ...
    var j = function j(u, w) {
        if (u.classList) {
            ...
            u.className = u.className.replace(/([^ ]+)[ ]*/g, function v(y, x) {
                if (x === w) {
                    return ""
                }
                return y
