/*
 * @Author: Tsang 
 * @Date: 2018-01-02 09:48:54 
 * @Last Modified by: Tsang
 * @Last Modified time: 2018-01-02 23:44:29
 */
/*
 * @Author: Tsang 
 * @Date: 2018-01-02 09:48:32 
 * @Last Modified by:   Tsang 
 * @Last Modified time: 2018-01-02 09:48:32 
 */
//收录了中国的省市区县数据  
const cityStr = [
...

    {
        ...
        "city": [
...

            {
                ...
                "letter": "Z",
                "hot": false,
                "area": ["石岐区", "东区", "西区", "南区", "五桂山区", "火炬高技术开发区", "板芙镇", "大涌镇", "东凤镇", "东升镇", "阜沙镇", "港口镇", "古镇镇", "横栏镇", "黄圃镇", "民众镇", "南朗镇", "南头镇", "三角镇", "三乡镇", "沙溪镇", "神湾镇", "坦洲镇", "小榄镇"]
            },
...
export default {
    ...
    //根据字母排序获取城市完整对象（省份，城市，区域的字段）
    getCitiesByLetterGroup: (letterGroup) => {
        if (letterGroup === '热门城市') {
            var citiesResult = [];
            var cities, i, j;
            for (i = 0; i < cityStr.length; i++) {
                cities = cityStr[i].city;
                let province = cityStr[i].name;
                for (j = 0; j < cities.length; j++) {
                    if (cities[j].hot) {
                        let temp = {};
                        temp.name = cities[j].name;
                        temp.area = cities[j].area;
                        temp.province = province;
                        citiesResult.push(temp);
                    }
            ...
            for (i = 0; i < cityStr.length; i++) {
                ...
                let province = cityStr[i].name;
                for (j = 0; j < cities.length; j++) {
                    for (k = 0; k < letterGroup.split('').length; k++) {
                        const letter = letterGroup.split('')[k];
                        if (letter === cities[j].letter) {
                            let temp = {};
                            temp.name = cities[j].name;
                            temp.area = cities[j].area;
                            temp.province = province;
