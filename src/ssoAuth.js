...
export default {
	...
	isPerson() {
		return 'STUDENT' === this.userInfo().type || 'PERSON' === this.userInfo().type;
	},
	
	isCompany() {
		return 'COMPANY' === this.userInfo().type;
	},
	...
	checkTicket(ticket) {
		...
		$.ajax({
			...
			success: function (data) {
				if (data) {
					data = data.loginSysUser;
					var info = {
						id: data.id,
						displayName: data.name,
						type: data.type,
						avatar: data.avatar?`${http.sftpPathPrefix}/${data.avatar}`:'/static/img/timg.jpg'
						// avatar: '/static/img/timg.jpg'
						
					}
					if (data.type === 'COMPANY') {
						info.displayName = data.company.sortName;
						if (data.company.logo){
							info.avatar = `${http.sftpPathPrefix}/${data.company.logo}`;
						}
